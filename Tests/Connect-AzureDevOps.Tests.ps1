...
Describe "Connect-AzureDevOps Function Tests" {
    ...
    Context "Parameter Validation" {
        ...
        It "Should have OrganizationUri as mandatory parameter" {
            ...
            $ParameterAttributes = $OrganizationUriParam.Attributes |
                Where-Object { $_ -is [System.Management.Automation.ParameterAttribute] } |
                ForEach-Object { $_.Mandatory }
            $ParameterAttributes | Should -Contain $true
        }

        It "Should have TenantId as mandatory parameter" {
            $Command = Get-Command Connect-AzureDevOps
            $TenantIdParam = $Command.Parameters['TenantId']
            $ParameterAttributes = $TenantIdParam.Attributes |
                Where-Object { $_ -is [System.Management.Automation.ParameterAttribute] } |
                ForEach-Object { $_.Mandatory }
            $ParameterAttributes | Should -Contain $true
        }
        ...
        It "Should have ClientId as mandatory parameter" {
            ...
            $ParameterAttributes = $ClientIdParam.Attributes |
                Where-Object { $_ -is [System.Management.Automation.ParameterAttribute] } |
                ForEach-Object { $_.Mandatory }
            $ParameterAttributes | Should -Contain $true
        }
        ...
        It "Should have ClientSecret as mandatory parameter" {
            ...
            $ParameterAttributes = $ClientSecretParam.Attributes |
                Where-Object { $_ -is [System.Management.Automation.ParameterAttribute] } |
                ForEach-Object { $_.Mandatory }
            $ParameterAttributes | Should -Contain $true
        }
        ...
        It "Should validate OrganizationUri format" {
            $ConnectParams = @{
                OrganizationUri = $script:UnitTestData.InvalidUri
                TenantId = $script:UnitTestData.TenantId
                ClientId = $script:UnitTestData.ClientId
                ClientSecret = $TestSecureString
                ErrorAction = 'SilentlyContinue'
            }
            { Connect-AzureDevOps @ConnectParams } | Should -Throw "*OrganizationUri must be in format*"
        }

        It "Should validate TenantId GUID format" {
            $ConnectParams = @{
                OrganizationUri = $script:UnitTestData.OrganizationUri
                TenantId = $script:UnitTestData.InvalidGuid
                ClientId = $script:UnitTestData.ClientId
                ClientSecret = $TestSecureString
                ErrorAction = 'SilentlyContinue'
            }
            { Connect-AzureDevOps @ConnectParams } | Should -Throw "*TenantId must be a valid GUID*"
        }
        ...
        It "Should validate ClientId GUID format" {
            $ConnectParams = @{
                OrganizationUri = $script:UnitTestData.OrganizationUri
                TenantId = $script:UnitTestData.TenantId
                ClientId = $script:UnitTestData.InvalidGuid
                ClientSecret = $TestSecureString
                ErrorAction = 'SilentlyContinue'
            }
            { Connect-AzureDevOps @ConnectParams } | Should -Throw "*ClientId must be a valid GUID*"
        }
        ...
        It "Should accept Project parameter and fail at OAuth2 authentication level" {
            ...
            $ConnectParams = @{
                ...
                ClientId = $script:UnitTestData.ClientId
                ClientSecret = $TestSecureString
                Project = $script:UnitTestData.Project
                ErrorAction = 'SilentlyContinue'
            }
            { Connect-AzureDevOps @ConnectParams } | Should -Throw "*Failed to acquire Azure DevOps access token*"
        }
        ...
        It "Should require SecureString for ClientSecret parameter" {
            ...
            { Connect-AzureDevOps @ConnectParams } | Should -Throw
        }
    }
